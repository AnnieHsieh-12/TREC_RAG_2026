...
export async function generateJsonWithRetry<T>(
  ...
  for (let jsonAttempt = 1; jsonAttempt <= maxJsonAttempts; jsonAttempt += 1) {
    ...
    for (let requestAttempt = 1; requestAttempt <= maxRequestAttempts; requestAttempt += 1) {
      ...
      // NCHC caches identical requests, so provider-error retries must vary
      // the request or they replay the same cached failure (see runner note).
      const requestMessages = requestAttempt === 1
        ? messages
        : [...messages, { role: "user" as const, content: `[retry ${requestAttempt}: previous attempt failed; return the JSON now]` }];
      try {
