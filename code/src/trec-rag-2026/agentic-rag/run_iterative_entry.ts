...
export function llmCfg(spec:string,env:NodeJS.ProcessEnv){
  ...
  if(provider==='openai')return{provider:'openai_llm',model,apiKeyEnv:'OPENAI_API_KEY',baseUrl:env.OPENAI_BASE_URL||'https://api.openai.com/v1',temperature:0,maxTokens:8192};
...
export function applyVersionModels(opts:IterativeOptions,models:{base?:string;writer?:string;query?:string},env:NodeJS.ProcessEnv=process.env):IterativeOptions{
  ...
  if(models.writer&&!opts.llmWriter)out.llmWriter=llmCfg(models.writer,env) as any;
  if(models.query&&!opts.llmQuery)out.llmQuery=llmCfg(models.query,env) as any;
  return out;
}
function str(v:unknown){return typeof v==='string'?v.trim():''} function req(v:unknown,n:string){const s=str(v); if(!s)throw new Error(`Missing ${n}`); return s;}
async function main(){console.log(JSON.stringify(await runIterativeAgenticRag(parse(process.argv.slice(2))),null,2));}
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href)main().catch(e=>{console.error(e instanceof Error?e.message:String(e));process.exitCode=1;});
