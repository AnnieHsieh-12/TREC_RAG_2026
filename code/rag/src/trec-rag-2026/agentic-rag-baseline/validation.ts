...
type ValidationArgs = {
  config: AgenticRagBaselineConfig;
  topic: TopicIdentity;
...
export function validateOfficialRagOutputObject(value: unknown): ValidationResult {
...
export function validateRagOutputObject(
  value: unknown,
  args: ValidationArgs,
  _legacyOptions: { allowUncitedReferences?: boolean } = {},
  ...
  if (issues.length > 0) {
    throw new AgenticRagValidationError(
      `Invalid RAG output object: ${issues.map((issue) => issue.code).join(",").slice(0, 300)}`,
      issues,
    );
  }
...
export function validateRagOutputObjectStrict(value: unknown, args: ValidationArgs): ValidationResult {
  const canonical = resolveDirectDocidCitations(value);
  const issues = [
    ...collectOfficialIssues(canonical),
    ...collectProducerPolicyIssues(canonical, args),
...
export function normalizeRagOutputObjectReferences(
  ...
  const normalized: AgenticRagOutputObject = {
    ...
    answer: validated.answer.map((sentence) => ({
      text: sentence.text,
      citations: [...new Set(sentence.citations.map((citation) => {
        const remapped = oldToNew.get(citation);
        if (remapped === undefined) {
          throw new AgenticRagValidationError("Citation could not be remapped.", [
...
function collectOfficialIssues(value: unknown): ValidationIssue[] {
  ...
  if (!isRecord(metadata)) {
    ...
    for (const key of ["team_id", "run_id", "narrative_id", "narrative", "run_desc"]) {
      if (typeof metadata[key] !== "string" || metadata[key].trim() === "") {
        issues.push({ code: "INVALID_METADATA_VALUE", message: `metadata.${key} must be non-empty.` });
      }
    }
  }
  ...
  answer.forEach((sentence, sentenceIndex) => {
    ...
    if (!Array.isArray(sentence.citations)) {
      issues.push({ code: "INVALID_CITATIONS", message: `answer[${sentenceIndex}].citations must be an array.` });
      return;
    }
    if (sentence.citations.length > 3) {
      issues.push({ code: "TOO_MANY_CITATIONS", message: `answer[${sentenceIndex}] has more than three citations.` });
    }
    ...
    sentence.citations.forEach((citation, citationIndex) => {
      ...
      if (identity === undefined) {
        issues.push({
          code: Number.isInteger(citation) ? "CITATION_OUT_OF_RANGE" : "INVALID_CITATION",
          message: `answer[${sentenceIndex}].citations[${citationIndex}] must be a valid reference index or exact docid.`,
        });
        return;
      }
      if (seen.has(identity)) {
        issues.push({ code: "DUPLICATE_SENTENCE_CITATION", message: `answer[${sentenceIndex}] repeats citation ${identity}.` });
      }
      seen.add(identity);
    });
  });
...
function collectProducerPolicyIssues(value: unknown, args: ValidationArgs): ValidationIssue[] {
  if (!isRecord(value) || !isRecord(value.metadata) || !Array.isArray(value.references)) return [];
  const issues: ValidationIssue[] = [];
  const metadata = value.metadata;

  // These fields/invariants describe our producer, not the organizer's minimum schema.
  requireKeys(metadata, ["type", "title", "prompt"], "producer metadata", issues);
  if (metadata.team_id !== args.config.teamId) {
    issues.push({ code: "TEAM_ID_MISMATCH", message: "metadata.team_id must match producer config." });
  }
  if (metadata.run_id !== args.config.runId) {
    issues.push({ code: "RUN_ID_MISMATCH", message: "metadata.run_id must match producer config." });
  }
  if (metadata.type !== "automatic") {
    issues.push({ code: "INVALID_TYPE", message: 'producer metadata.type must equal "automatic".' });
  }
  if (metadata.narrative_id !== args.topic.qid) {
    issues.push({ code: "NARRATIVE_ID_MISMATCH", message: "metadata.narrative_id must preserve qid." });
  }
  if (metadata.title !== "") {
    issues.push({ code: "INVALID_DEV_TITLE", message: 'producer metadata.title must equal "" in dev mode.' });
  }
  if (metadata.narrative !== args.topic.narrative) {
    issues.push({ code: "NARRATIVE_MISMATCH", message: "metadata.narrative must preserve the topic narrative." });
  }
  if (metadata.prompt !== args.config.promptVersion) {
    issues.push({ code: "PROMPT_VERSION_MISMATCH", message: "metadata.prompt must match producer config." });
  }
  value.references.forEach((reference, index) => {
    if (typeof reference === "string" && !args.readDocids.has(reference)) {
      issues.push({ code: "REFERENCE_NOT_READ", message: `references[${index}] was not read by the producer.` });
    }
  });
  return issues;
}
...
function resolveDirectDocidCitations(value: unknown): unknown {
  if (!isRecord(value) || !Array.isArray(value.references) || !Array.isArray(value.answer)) return value;
  const references = value.references;
  if (!references.every((reference) => typeof reference === "string")) return value;
  const indexByDocid = new Map<string, number>();
  references.forEach((reference, index) => indexByDocid.set(reference as string, index));
  return {
    ...value,
    answer: value.answer.map((sentence) => {
      if (!isRecord(sentence) || !Array.isArray(sentence.citations)) return sentence;
      return {
        ...sentence,
...
function sanitizeModelDraft(value: unknown, readDocids: Set<string>): unknown {
  ...
  if (!isRecord(resolved) || !Array.isArray(resolved.references) || !Array.isArray(resolved.answer)) return resolved;
  if (!resolved.references.every((reference) => typeof reference === "string")) return resolved;
  ...
  return {
    ...
    answer: resolved.answer.map((sentence) => {
      ...
      for (const citation of sentence.citations) {
        ...
        if (mapped !== undefined && !citations.includes(mapped)) citations.push(mapped);
      }
...
function validateReferences(references: unknown[], issues: ValidationIssue[]): string[] {
  const result: string[] = [];
  const seen = new Set<string>();
  references.forEach((reference, index) => {
    ...
    if (reference !== reference.trim() || !CLIMBMIX_DOCID_RE.test(reference)) {
      issues.push({ code: "INVALID_CLIMBMIX_DOCID", message: `references[${index}] is not an exact ClimbMix docid.` });
    }
    if (seen.has(reference)) {
      issues.push({ code: "DUPLICATE_REFERENCE", message: `Duplicate reference: ${reference}` });
    }
    seen.add(reference);
...
function citationIdentity(citation: unknown, references: string[]): string | undefined {
  if (Number.isInteger(citation)) {
    const index = citation as number;
    return index >= 0 && index < references.length ? references[index] : undefined;
  }
  if (typeof citation === "string" && references.includes(citation)) return citation;
  return undefined;
}
...
function requireKeys(
  ...
  for (const key of expectedKeys) {
    if (!Object.hasOwn(value, key)) {
      issues.push({ code: "MISSING_FIELD", message: `Missing field in ${label}: ${key}` });
    }
  }
}
