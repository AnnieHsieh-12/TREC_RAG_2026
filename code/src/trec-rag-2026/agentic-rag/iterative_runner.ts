...
import { ExposureLedger, enforceAnswerPlan, LedgerViolation } from "./evidence_plan";
...
facet_queries:false,facet_span_autofix:false,facet_depth:1000,splice_head_keep:200,
...
aspect_writer:false,
evidence_plan:false,evidence_plan_policy:"strict",evidence_plan_max_retries:3,evidence_plan_max_tokens:16384};
...
if(POLICY.enumerative_style&&!POLICY.evidence_ledger&&!POLICY.evidence_plan)throw new Error("enumerative_style requires evidence_ledger or evidence_plan");
if(POLICY.evidence_plan&&POLICY.evidence_ledger)throw new Error("evidence_plan and evidence_ledger are mutually exclusive");
if(POLICY.atomic_sentences&&POLICY.enumerative_style)throw new Error("atomic_sentences and enumerative_style are mutually exclusive");
if(POLICY.dense_after_aspect&&!POLICY.dense_writing)throw new Error("dense_after_aspect requires dense_writing");
if(POLICY.dense_after_aspect&&POLICY.per_aspect_generation)throw new Error("dense_after_aspect and per_aspect_generation are mutually exclusive"); const env=o.env??process.env,out=resolve(o.outputDir); if(o.force)rmSync(out,{recursive:true,force:true}); mkdirSync(join(out,"topics"),{recursive:true}); const llmCfg=normalizeLlmClientConfig(o.llm),llm=createLlmClient(llmCfg,env);
...
async function answerOneAspect(a:{topic:TopicIdentity;o:IterativeOptions;llm:LlmClient;out:string;env:NodeJS.ProcessEnv;summary:any},aspect:string,docidText:Map<string,string>,shared:Map<string,ReadDoc>):Promise<{sentences:{text:string;docids:string[]}[];docsUsed:number}>{
  ...
  if(POLICY.aspect_ce_rerank&&hits.length>1){
    ...
    }catch{/* Keep BM25 order if cross-encoder scoring is unavailable. */} }
  }
  ...
  if(POLICY.integrate_answer&&groups.length>0&&capped.length>0){
    try{
      ...
      if(blocks.length>1){
        ...
        else console.error(`  !! ${a.topic.qid}: integration reduced coverage (${capped.length} sentences/${before} words -> ${merged.length} sentences/${after} words); keeping the unintegrated draft`);
      }
    }catch(e){console.error(`  !! ${a.topic.qid}: integration failed (${e instanceof Error?e.message.slice(0,80):String(e)}); keeping the unintegrated draft`);}
  }
...
if(POLICY.evidence_plan){
  ...
  for(let attempt=1;attempt<=POLICY.evidence_plan_max_retries&&!draft;attempt++){
    try{
      const raw=await generateJsonWithRetry({client:a.llms.writer,messages:[{role:"user",content:buildLedgerAnswerPrompt({topic:a.topic,documents:denseDocs,subquestions:subs,...(violation?{violation}:{}),...(POLICY.enumerative_style?{enumerative:{minWords:POLICY.enumerative_words_min,maxWords:POLICY.enumerative_words_max}}:{})})}],temperature:0,maxTokens:POLICY.evidence_plan_max_tokens,validate:validateLedgerPlan,stage:"answer_generation",maxRequestRetries:3,onAttempt:(at)=>recordAttempt({attempt:at,stage:"answer_generation",qid:a.topic.qid,out:a.out,env:a.env,summary:a.summary})});
      ...
      const verified=enforceAnswerPlan({ledger,answerPlan:plan,subquestions:subqs,policy:POLICY.evidence_plan_policy as "basic"|"strict"});
      ...
      ledgerTrace={mode:"evidence_plan",policy:POLICY.evidence_plan_policy,attempts:attempt,sentences:verified.length,evidence_records:ledger.records.length};
    }catch(e){
      if(e instanceof LedgerViolation){violation=e.message; ledgerTrace={mode:"evidence_plan_rejected",attempts:attempt,violation};}
      else {ledgerTrace={mode:"evidence_plan_error",attempts:attempt,error:redact(e instanceof Error?e.message:String(e),a.env)};}
    }
  }
}
...
if(verifyStats&&verifyStats.mode==="evidence_ledger")verifyStats={...verifyStats,ledger_then:"reattribute_or_verify"};
...
writeJson(join(a.out,"topics",`${a.topic.qid}.gen_trace.json`),{topic_id:a.topic.qid,aspects,aspect_count:aspects.length,aspect_decomposition_ok:aspects.length>0,per_aspect:perAspectTrace,nuggets:nuggetTrace,evidence_plan:ledgerTrace,verify:verifyStats,style:sentenceStyleStats(sanitized)});}
...
async function search(o:IterativeOptions,query:string,depth:number,env:NodeJS.ProcessEnv,sink?:Map<string,string>){
  ...
  for(const q of tries){
    if(q!==query) console.error("search: retrying with the first 200 query characters");
    for(let d=depth; d>=Math.min(depth,313); d=Math.floor(d/2)){
      try{ return await searchAtDepth(o,q,d,env,sink); }
      catch(e){ lastErr=e;
        if(!giant(e)) throw e;
        console.error(`search: oversized response (${String((e as any)?.message??e).slice(0,40)}); halving depth ${d}`);
      }
    }
  }
  throw lastErr;
