...
} from "../src/trec-rag-2026/agentic-rag-baseline/validation";

const fixture = JSON.parse(
  readFileSync(new URL("../../test-fixtures/rag_validation_cases.json", import.meta.url), "utf8"),
);
const args = {
  config: {
    runId: "test-run",
    ...
    runDesc: "validator contract fixture",
    layersDesc: "fixture",
  },
...
test("official rules accept empty citations, uncited references, and extra metadata", () => {
  assert.deepEqual(validateOfficialRagOutputObject(fixture), { ok: true, issues: [] });
});
...
test("direct-docid citations are accepted and canonicalized to indices", () => {
  ...
  assert.deepEqual(validateRagOutputObject(value, args).answer[0].citations, [1]);
});
...
test("1024 words pass and 1025 words fail", () => {
  ...
  assert.ok(validateOfficialRagOutputObject(fail).issues.some((issue) => issue.code === "ANSWER_TOO_LONG"));
});

test("invalid docids and out-of-range citations fail", () => {
  const badDocid = clone(fixture);
  badDocid.references[0] = "climbmix-1";
  assert.ok(validateOfficialRagOutputObject(badDocid).issues.some((issue) => issue.code === "INVALID_CLIMBMIX_DOCID"));
  const badCitation = clone(fixture);
  badCitation.answer[0].citations = [99];
  assert.ok(validateOfficialRagOutputObject(badCitation).issues.some((issue) => issue.code === "CITATION_OUT_OF_RANGE"));
});
...
test("the organizer limit is at most three citations per answer item", () => {
  ...
  assert.ok(validateOfficialRagOutputObject(value).issues.some((issue) => issue.code === "TOO_MANY_CITATIONS"));
});
